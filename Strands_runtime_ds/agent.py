...
MODEL_ID = os.environ.get("MODEL_ID", deepseek_openai.DEFAULT_MODEL_ID).strip()
DEEPSEEK_API_KEY = os.environ.get("DEEPSEEK_API_KEY", "").strip()
DEEPSEEK_BASE_URL = os.environ.get(
    "DEEPSEEK_BASE_URL", deepseek_openai.DEFAULT_BASE_URL
).strip()
DEEPSEEK_REASONING_EFFORT = deepseek_openai.validate_reasoning_effort(
    os.environ.get("DEEPSEEK_REASONING_EFFORT", "high")
)
DEEPSEEK_MAX_OUTPUT_TOKENS = deepseek_openai.validate_max_output_tokens(
...
MODEL_PRICING_LABEL = os.environ.get(
    "MODEL_PRICING_LABEL",
    "deepseek-v4-flash-official-2026-08",
).strip() or "deepseek-v4-flash-official-2026-08"
...
class InvocationRequest:
    ...
    def from_payload(cls, payload: dict, context: Any = None) -> "InvocationRequest":
        ...
        return cls(
            ...
            # Existing OpenAI-compatible callers expect Runtime message payloads
            # to stream even when they omit a downstream `stream` flag.
...
def _model_usage_payload(
    ...
    *,
    ...
    model = getattr(runtime_agent, "model", None)
    reasoning_tokens = int(getattr(model, "reasoning_tokens", 0) or 0)
...
def _make_model() -> deepseek_openai.DeepSeekResponsesModel:
    """Create one stateless DeepSeek Responses model for an invocation."""
    return deepseek_openai.DeepSeekResponsesModel(
        model_id=MODEL_ID,
        api_key=DEEPSEEK_API_KEY,
        base_url=DEEPSEEK_BASE_URL,
        reasoning_effort=DEEPSEEK_REASONING_EFFORT,
        max_output_tokens=DEEPSEEK_MAX_OUTPUT_TOKENS,
        connect_timeout_seconds=MODEL_CONNECT_TIMEOUT_SECONDS,
        read_timeout_seconds=MODEL_READ_TIMEOUT_SECONDS,
...
def _prepare(request: InvocationRequest):
    ...
    # Validate model configuration before acquiring Memory, Gateway, or Code
    # Interpreter resources. A deployment placeholder must fail locally.
    model = _make_model()
...
async def stream(request: InvocationRequest) -> AsyncIterator[dict]:
    """Stream OpenAI-compatible chunks for a Runtime caller."""
    started_at = time.perf_counter()
    succeeded = False
    runtime_agent = None
    interpreter_session = None
