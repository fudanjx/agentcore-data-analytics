...
from bedrock_mantle_openai import BedrockMantleOpenAIResponsesModel
...
MODEL_PROVIDER = os.environ.get("MODEL_PROVIDER", "bedrock_mantle_openai").strip().lower()
if MODEL_PROVIDER != "bedrock_mantle_openai":
    raise ValueError("MODEL_PROVIDER must be 'bedrock_mantle_openai' for Strands_runtime_gpt")
...
PROMPT_CACHE_KEY_PREFIX = (
    os.environ.get("PROMPT_CACHE_KEY_PREFIX", "strands-runtime-gpt").strip()
    or "strands-runtime-gpt"
)
...
PROMPT_CACHE_TTL = os.environ.get("PROMPT_CACHE_TTL", "30m").strip().lower() or "30m"
if PROMPT_CACHE_TTL != "30m":
    raise ValueError("PROMPT_CACHE_TTL must be '30m' for GPT-5.6 Luna explicit caching")
...
MODEL_CACHE_WRITE_PRICE_PER_MTOK_USD = _price_env(
...
def _model_usage_payload(
    ...
    *,
    ...
    cache_write_rate = MODEL_CACHE_WRITE_PRICE_PER_MTOK_USD
...
def _make_model() -> BedrockMantleOpenAIResponsesModel:
    """Create the GPT-5.6 Luna Responses model without Mantle conversation state."""
    if not OPENAI_PROJECT_ID:
        raise ValueError("OPENAI_PROJECT_ID must be configured for Bedrock Mantle")
    if not MODEL_REGION:
        raise ValueError("MODEL_REGION must be configured for Bedrock Mantle")
    return BedrockMantleOpenAIResponsesModel(
        model_id=MODEL_ID,
